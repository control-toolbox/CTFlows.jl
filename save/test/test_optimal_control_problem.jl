function test_optimal_control_problem()
    ...
    * = CTFlows.:(*)
    ...
    @testset "Augmented flow: with control" begin
        ...
        CTModels.state!(pre_ocp, 2)
        CTModels.control!(pre_ocp, 1)
        dynamics!(r, t, x, u, v) = r .= [x[2], u[1] + v[1]]
        CTModels.dynamics!(pre_ocp, dynamics!)
        lagrange(t, x, u, v) = 0.5 * u[1]^2
        CTModels.objective!(pre_ocp, :min; lagrange=lagrange)
        definition = quote end
    ...
    @testset "Augmented flow: scalar vs vector variable" begin
        ...
        CTModels.variable!(pre_ocp1, 1)  # Scalar
        CTModels.time!(pre_ocp1; t0=0.0, tf=1.0)
        CTModels.state!(pre_ocp1, 1)
        dynamics!(r, t, x, u, v) = r .= v[1] * x[1]
        CTModels.dynamics!(pre_ocp1, dynamics!)
        lagrange(t, x, u, v) = x[1]^2
        CTModels.objective!(pre_ocp1, :min; lagrange=lagrange)
        definition = quote end
        ...
        CTModels.time!(pre_ocp2; t0=0.0, tf=1.0)
        CTModels.state!(pre_ocp2, 1)
        dynamics2!(r, t, x, u, v) = r .= (v[1] + v[2]) * x[1]
        CTModels.dynamics!(pre_ocp2, dynamics2!)
        lagrange2(t, x, u, v) = x[1]^2
        CTModels.objective!(pre_ocp2, :min; lagrange=lagrange2)
    ...
    @testset "Augmented flow: error on Fixed model" begin
        ...
        try
            ...
            Test.@test e isa CTBase.Exceptions.PreconditionError
            Test.@test occursin("augment=true requires", e.msg)
    ...
    @testset "Augmented flow: error on trajectory call" begin
        ...
        try
            ...
            Test.@test e isa CTBase.Exceptions.PreconditionError
            Test.@test occursin("point evaluation", e.msg)
    ...
    @testset "Augmented flow: mathematical correctness" begin
        ...
        dynamics!(r, t, x, u, v) = r .= v[1] * x[1]
        CTModels.dynamics!(pre_ocp, dynamics!)
        lagrange(t, x, u, v) = 0.0  # No Lagrange cost
        CTModels.objective!(pre_ocp, :min; lagrange=lagrange)
        definition = quote end
        CTModels.definition!(pre_ocp, definition)
        CTModels.time_dependence!(pre_ocp; autonomous=true)
    ...
    @testset "Augmented flow: dimension consistency" begin
        ...
        pre_ocp = CTModels.PreModel()
        CTModels.variable!(pre_ocp, 3)  # 3D variable
        CTModels.time!(pre_ocp; t0=0.0, tf=1.0)
        CTModels.state!(pre_ocp, 2)
