...
function __concat_jumps(F::AbstractFlow, G::AbstractFlow, jump::Union{Nothing, Tuple{Time, Any}}=nothing)
    jumps = F.jumps
...
function concatenate(F::TF, g::Tuple{ctNumber, TF})::TF where {TF<:OptimalControlFlow}
    ...
    jumps      = __concat_jumps(F, G) # jumps
    return TF(F.f, rhs!, feedback_u, F.ocp, tstops, jumps) # we choose default values and options of F
