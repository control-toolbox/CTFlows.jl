# ---------------------------------------------------------------------------------------------------
#
struct HamiltonianFlow{D, U, T} <: AbstractFlow{D, U, T}
    f::Function      # f(args..., rhs): compute the flow
    rhs!::Function   # OrdinaryDiffEq rhs
    tstops::Times    # stopping times
    HamiltonianFlow{D, U, T}(f, rhs!) where {D, U, T} = new{D, U, T}(f, rhs!, Vector{Time}())
    HamiltonianFlow{D, U, T}(f, rhs!, tstops) where {D, U, T} = new{D, U, T}(f, rhs!, tstops)
end
...
function hamiltonian_usage(alg, abstol, reltol, saveat; kwargs_Flow...)
    ...
    function f(t0::Time, x0::State, p0::Costate, tf::Time, v::Variable=__variable(); _t_stops_interne, DiffEqRHS, tstops=__tstops(), kwargs...)
        sol = f((t0, tf), x0, p0, v; _t_stops_interne=_t_stops_interne, DiffEqRHS=DiffEqRHS, tstops=tstops, kwargs...)
        n = size(x0, 1)
...
function Flow(h::Hamiltonian; alg=__alg(), abstol=__abstol(), 
    ...
    return HamiltonianFlow{DCoTangent, CoTangent, Time}(f, rhs!)
