...
- **`SciMLIntegrator <: AbstractIntegrator`** — defined in the package extension
  `CTFlowsSciML`, loaded when `SciMLBase` and `OrdinaryDiffEqCore` are available. The
...
- **Solution wrapper with getters** — Phase 1 returns the raw SciML `ODESolution`.
  Later, `build_solution` for `StateTrajectoryConfig` will produce a CTFlows-specific
