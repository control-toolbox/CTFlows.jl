...
function test_macro_helpers_dg()
    ...
    Test.@testset "__transform_brackets" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        result = DifferentialGeometry.__transform_brackets(quote
            [a, b]
        end, opts)
        call = only(_exprs(result))
        Test.@test @capture(
            call,
            CTFlows.DifferentialGeometry._lie_mac(
        ...
        Test.@test @capture(
            call,
            CTFlows.DifferentialGeometry._poisson_mac(
                _, _, CTBase.Traits.Autonomous, CTBase.Traits.Fixed, _, _, _
            )
        )
        ...
        local inner_arg
        matched_outer = @capture(
            outer, CTFlows.DifferentialGeometry._lie_mac(inner_arg_, _, _, _, _, _, _)
        )
        Test.@test matched_outer
        Test.@test @capture(
            inner_arg, CTFlows.DifferentialGeometry._lie_mac(_, _, _, _, _, _, _)
        )
        ...
        result2 = DifferentialGeometry.__transform_brackets(quote
            [a, b]
        end, opts2)
    ...
    Test.@testset "_as_ham" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@test DifferentialGeometry._as_ham(ham, Traits.Autonomous, Traits.Fixed) ===
            ham
    end
    ...
    Test.@testset "_check_td" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@test DifferentialGeometry._check_td(_f1, Traits.Autonomous, Val{false}()) ===
            nothing
        Test.@test DifferentialGeometry._check_td(
            _f1, Traits.NonAutonomous, Val{true}()
        ) === nothing

        # Val{false} (no override) → always nothing, regardless of declared TD
        Test.@test DifferentialGeometry._check_td(
            vf_auto, Traits.Autonomous, Val{false}()
        ) === nothing
        Test.@test DifferentialGeometry._check_td(
            vf_auto, Traits.NonAutonomous, Val{false}()
        ) === nothing
        Test.@test DifferentialGeometry._check_td(
            vf_nonaut, Traits.Autonomous, Val{false}()
        ) === nothing

        # Val{true} + consistent TD → nothing
        Test.@test DifferentialGeometry._check_td(
            vf_auto, Traits.Autonomous, Val{true}()
        ) === nothing
        Test.@test DifferentialGeometry._check_td(
            vf_nonaut, Traits.NonAutonomous, Val{true}()
        ) === nothing
    ...
    Test.@testset "_check_vd" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@test DifferentialGeometry._check_vd(_f1, Traits.Fixed, Val{false}()) ===
            nothing
        Test.@test DifferentialGeometry._check_vd(_f1, Traits.NonFixed, Val{true}()) ===
            nothing

        # Val{false} → always nothing
        Test.@test DifferentialGeometry._check_vd(vf_fixed, Traits.Fixed, Val{false}()) ===
            nothing
        Test.@test DifferentialGeometry._check_vd(
            vf_fixed, Traits.NonFixed, Val{false}()
        ) === nothing
        Test.@test DifferentialGeometry._check_vd(
            vf_nonfixed, Traits.Fixed, Val{false}()
        ) === nothing

        # Val{true} + consistent VD → nothing
        Test.@test DifferentialGeometry._check_vd(vf_fixed, Traits.Fixed, Val{true}()) ===
            nothing
        Test.@test DifferentialGeometry._check_vd(
            vf_nonfixed, Traits.NonFixed, Val{true}()
        ) === nothing
