...
FakeADBackend() = FakeADBackend(CTBase.Strategies.StrategyOptions())
...
function test_ad_backend()
    Test.@testset "AD Backend Tests" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "Unit: AbstractADBackend abstract type" begin
            ...
            Test.@test backend isa CTBase.Strategies.AbstractStrategy
        end
        ...
        Test.@testset "Unit: DifferentiationInterface construction" begin
            ...
            metadata = CTBase.Strategies.metadata(Differentiation.DifferentiationInterface)
        ...
        Test.@testset "Unit: CTBase.Strategies contract" begin
            # id
            Test.@test CTBase.Strategies.id(Differentiation.DifferentiationInterface) ===
                  :di

            # description
            desc = CTBase.Strategies.description(Differentiation.DifferentiationInterface)
            Test.@test desc isa String
            Test.@test !isempty(desc)

            # metadata
            metadata = CTBase.Strategies.metadata(Differentiation.DifferentiationInterface)
            Test.@test metadata isa CTBase.Strategies.StrategyMetadata
